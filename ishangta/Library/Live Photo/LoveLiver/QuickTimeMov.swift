...
//                logAR.message("not found video track")
            ...
            let readerSettings: [String: AnyObject] = [
                kCVPixelBufferPixelFormatTypeKey as String: kCVPixelFormatType_32ARGB as AnyObject
            ]
...
//                    logAR.message("cant add audio reader")
            ...
            // write video track
            let media_queue = DispatchQueue(label: "assetVideoWriterQueue", attributes: [])
            input.requestMediaDataWhenReady(on: media_queue) {
                        ...
                        if reader.status == .completed && aAudioAsset.tracks.count > 1 {
                            ...
                            audioWriterInput?.requestMediaDataWhenReady(on: media_queue) {
                                while (audioWriterInput?.isReadyForMoreMediaData)! {
                                    if let sampleBuffer2:CMSampleBuffer = audioReaderOutput?.copyNextSampleBuffer() {
                                        if audioReader?.status == .reading {
                                            audioReader?.cancelReading()
//                                            if CMSampleBufferDataIsReady(sampleBuffer2) {
//                                                if !(audioWriterInput?.append(sampleBuffer2) ?? true) {
//                                                    audioReader?.cancelReading()
//                                                }
//                                            }
                                        } else {
