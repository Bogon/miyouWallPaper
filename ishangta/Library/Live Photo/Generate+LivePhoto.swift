...
class LivePhotoGenerator {
    ...
    func generate(livePhoto video: URL?, _ finished: ((_ status: Bool, _ photo: PHLivePhotoPlus?, _ pairedVideoPath: URL?, _ keyFramePath: URL?) -> Void)? = nil) {
        livePhotoQueue.async {
            ...
            let time = NSValue(time: CMTimeMultiplyByFloat64(asset.duration, multiplier: 1))
...
//                            finished?(false, finalPhoto, finalPhoto.pairedVideoPath, finalPhoto.keyPhotoPath)
    ...
    func newPath(for JPEG: Bool, and live: Bool) -> URL {
        ...
        let livePhotosFolder = "\(documentsDirectory)"
...
//            logAR.message("An error occurred while rendering the live photo: \(error)")
            return URL(fileURLWithPath: "\(documentsDirectory)/\(formatter.string(from: date)).jpg", isDirectory: false)
        }
        
        if JPEG && live {
            return URL(fileURLWithPath: "\(livePhotosFolder)/\(formatter.string(from: date)).jpg", isDirectory: false)
        } else if JPEG && !live {
            return URL(fileURLWithPath: "\(documentsDirectory)/\(formatter.string(from: date)).jpg", isDirectory: false)
        } else {
            return URL(fileURLWithPath: "\(livePhotosFolder)/\(formatter.string(from: date)).mov", isDirectory: false)
        }
    }
}
