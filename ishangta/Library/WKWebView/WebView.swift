...
private let WebviewWidth: CGFloat = UIScreen.main.bounds.size.width
private let WebviewHeight: CGFloat = UIScreen.main.bounds.size.height
...
class WebView: UIView {
    ...
    // 保存请求链接
    fileprivate var snapShotsArray: [Any]?
    ...
    fileprivate func setupUI(webConfig: WkwebViewConfig) {
        ...
        progressView.frame = CGRect(x: 0, y: 0, width: WebviewWidth, height: webConfig.progressHeight)
    ...
    func webloadType(_ target: AnyObject, _ loadType: WkwebLoadType) {
        ...
        switch loadType {
        case let .URLString(urltring):
            let urlstr = URL(string: urltring)
            let request = URLRequest(url: urlstr!)
            webView.load(request)

        case let .HTMLName(string):
            loadHost(string: string)

        case let .POST(string, parameters: postString):
            needLoadJSPOST = true
            // 给每一个key，value前后加上一个“
    ...
    // 请求链接处理
    fileprivate func pushCurrentSnapshotView(_ request: NSURLRequest) {
        // 连接是否为空
...
extension WebView: WKScriptMessageHandler {
    func userContentController(_: WKUserContentController, didReceive message: WKScriptMessage) {
        if let scriptMessage = webConfig?.scriptMessageHandlerArray {
            delegate?.webViewUserContentController(scriptMessage, didReceive: message)
        }
    }
}
    ...
    func webView(_ webView: WKWebView, decidePolicyFor navigationAction: WKNavigationAction, decisionHandler: @escaping (WKNavigationActionPolicy) -> Void) {
        ...
        if let requestURL = navigationURL?.removingPercentEncoding {
            ...
            if requestURL.hasPrefix("tel://") {
                ...
                // 用这个API打电话
                if let mobileURL = URL(string: requestURL) {
