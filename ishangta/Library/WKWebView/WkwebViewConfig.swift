...
struct WkwebViewConfig {
    ...
    /// 默认颜色
    public var progressTrackTintColor = UIColor.clear

    /// 加载颜色
    public var progressTintColor = UIColor.green
}
...
extension WKWebViewDelegate {
    /// 服务器开始请求的时候调用
    func webView(_: WKWebView, decidePolicyFor _: WKNavigationAction, decisionHandler _: @escaping (WKNavigationActionPolicy) -> Void) {}
    ...
    /// 执行JS注入方法
    func webViewUserContentController(_: [String], didReceive _: WKScriptMessage) {}

    /// JS执行回调方法
    func webViewEvaluateJavaScript(_: Any?, error _: Error?) {}
}
